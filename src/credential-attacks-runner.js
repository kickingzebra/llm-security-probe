...
const credPlugin = require('./credential-attacks-plugin');

const { generate: defaultGenerate } = ollamaClient;
const { PROMPTS, evaluate, PLUGIN_ID, CATEGORY } = credPlugin;

async function runCredentialAttacksSuite(options = {}) {
  const {
    model,
    baseUrl,
    fetchImpl,
