...
const privescPlugin = require('./privilege-escalation-plugin');

const { generate: defaultGenerate } = ollamaClient;
const { PROMPTS, evaluate, PLUGIN_ID, CATEGORY } = privescPlugin;

async function runPrivilegeEscalationSuite(options = {}) {
  const {
    model,
    baseUrl,
    fetchImpl,
